...
public class MockThreePairInfo extends PairInfo {
    public Map<Section, List<Pair>> value;
    public int tryCount;

    public MockThreePairInfo() {
        super();
        value = new HashMap<>();
        Section section = new Section(Course.BACKEND, Level.LEVEL1, Mission.CAR);
        Pair pair1 = new Pair("향미", "가의");
        pair1.add("민수");
        Pair pair2 = new Pair("지훈", "종현");
        value.put(section, List.of(pair1, pair2));
    }
    ...
    protected List<Pair> makePairs(Section section) {
        return List.of(new Pair("향미", "종현"), new Pair("가의", "민수"));
    }
